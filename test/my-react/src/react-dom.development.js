...
function createUpdateQueue(baseState) {
  var queue = {
    baseState: baseState, // 每次操作更新之后计算出的 state，作为下一次计算的基础
...
function createUpdate(expirationTime) {
  return {
    ...
    tag: UpdateState, // 四个状态，更新 updateState 0、替换 replaceState 1、强制 forceUpdate 2、throw 捕获 captureUpdate 3
...
function scheduleWork(fiber, expirationTime) {
   ...
   * 
   * nextRenderExpirationTime： 在是新的 renderRoot 的时候会被设置为当前任务的 expirationTime，
...
function legacyRenderSubtreeIntoContainer(parentComponent, children, container, forceHydrate, callback) {
  ...
  if (!root) {
    // Initial mount
    /**
     * legacyCreateRootFromDOMContainer 会创建一个 ReactRoot 对象
     */
...
var ReactDOM = {
  ...
  /**
   * render 是用在浏览器环境内的，hydrate 用在服务器环境下
   */
